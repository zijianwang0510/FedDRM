...
class FedPAC():
    ...
    def local_train(self, i, t):
        ...
        global_backbone_params = [p.clone().detach() for n, p in self.model.named_parameters() if 'image_classifier' not in n]
        ...
        head_params = [p for n, p in self.model.named_parameters() if 'image_classifier' in n]

        optimizer = optim.SGD(head_params, lr=self.get_lr(t), momentum=self.cfg.momentum, weight_decay=self.cfg.weight_decay)
        ...
        backbone_params = [p for n, p in self.model.named_parameters() if 'image_classifier' not in n]

        optimizer = optim.SGD(backbone_params, lr=self.get_lr(t), momentum=self.cfg.momentum, weight_decay=self.cfg.weight_decay)
        ...
        for _ in range(backbone_steps):
            ...
            proximal_term = 0.0
            current_backbone_params = [p for name, p in self.model.named_parameters() if 'image_classifier' not in name]
            for local_p, global_p in zip(current_backbone_params, global_backbone_params):
                proximal_term += (local_p - global_p).pow(2).sum()

            loss += (self.cfg.fedpac_mu / 2) * proximal_term
