...
class FedBABU():
        ...
        for t in range(1, self.cfg.commu_round + 1):
            ...
            if t <= self.cfg.commu_round - self.cfg.ft_round:
                for i in range(self.cfg.num_clients):
                    train_loss = self.local_train_backbone(i, t)
                ...
                for i in range(self.cfg.num_clients):
                    train_loss = self.local_train_head(i, t)
                    train_losses.append(train_loss)
    ...
    def local_train_backbone(self, i, t):
    ...
    def local_train_head(self, i, t):
        self.model.load_state_dict(torch.load(self.local_model_path[i], weights_only=True))

        self.model.train()

        head = [p for name, p in self.model.named_parameters() if 'image_classifier' in name]

        optimizer = optim.SGD(head, lr=self.get_lr(t), momentum=self.cfg.momentum, weight_decay=self.cfg.weight_decay)
