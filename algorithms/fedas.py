...
from utils.utils import get_log_path, select_model, get_global_model_path, get_local_model_path, get_record_path, get_prev_local_model_path
...
class FedAS():
    def __init__(self, cfg, train_loaders, test_loaders, num_data, logfile):
        ...
        self.prev_model = select_model(cfg)
        self.local_model_path = [get_local_model_path(cfg, i) for i in range(cfg.num_clients)]
        self.prev_local_model_path = [get_prev_local_model_path(cfg, i) for i in range(cfg.num_clients)]
        self.global_model_path = get_global_model_path(cfg)
        self.criterion = nn.CrossEntropyLoss()
        self.aligned_criterion = nn.MSELoss()
        ...
        for i in range(cfg.num_clients):
            ...
            torch.save(self.model.state_dict(), self.prev_local_model_path[i])
    ...
    def local_train(self, i, t):
        aligned_steps = self.cfg.local_steps // 5
        train_steps = self.cfg.local_steps - aligned_steps

        # --- Phase 1: Parameter Alignment ---
        self.model.load_state_dict(torch.load(self.local_model_path[i], weights_only=True))
        self.model.train()

        self.prev_model.load_state_dict(torch.load(self.prev_local_model_path[i], weights_only=True))
        self.prev_model.eval()

        backbone = [p for name, p in self.model.named_parameters() if 'image_classifier' not in name]

        optimizer = optim.SGD(backbone, lr=self.get_lr(t), momentum=self.cfg.momentum, weight_decay=self.cfg.weight_decay)

        train_iter = iter(self.train_loaders[i])

        for _ in range(aligned_steps):
            try:
            ...
            with torch.no_grad():
                prev_features = self.prev_model(images, FEATURE_ONLY=True)

            features = self.model(images, FEATURE_ONLY=True)

            loss = self.aligned_criterion(features, prev_features)

            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

        # --- Phase 2: Standard Local Training ---
    ...
    def aggregate(self):
        ...
        for i in range(self.cfg.num_clients):
            w = torch.load(self.prev_local_model_path[i], weights_only=True)
    ...
    def spread_model(self):
        ...
        for i in range(self.cfg.num_clients):
            local_model = torch.load(self.prev_local_model_path[i], weights_only=True)
